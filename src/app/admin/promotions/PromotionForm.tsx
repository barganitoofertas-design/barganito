...
export default function PromotionForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const discountPercentage = data.get("discountPercentage") as string;
    const description = data.get("description") as string;
    const expiresAt = data.get("expiresAt") as string;

    if (!productId) {
      setMessage({ type: "error", text: "Por favor, selecione um produto." });
      setLoading(false);
      return;
    }
    ...
    if (!expiresAt) {
      setMessage({
        type: "error",
        text: "Por favor, defina a data de expiração.",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: "error",
        text:
          `Erro ao ${initialData ? "atualizar" : "cadastrar"} promoção: ` +
  ...
  return (
    ...
    >
      ...
      <h2 style={{ marginBottom: "1.5rem" }}>
        {initialData ? "Editar Promoção" : "Cadastrar Nova Promoção"}
      </h2>

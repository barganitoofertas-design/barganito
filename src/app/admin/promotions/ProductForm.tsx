...
export default function ProductForm({
      ...
      if (metadata) {
        setFormData((prev) => ({
          ...
          currentPrice: metadata.price
            ? metadata.price.toFixed(2).replace(".", ",")
            : prev.currentPrice,
          ...(metadata.exists && metadata.product
            ? {
                currentPrice: (metadata.product as any).currentPrice
                  .toFixed(2)
                  .replace(".", ","),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!url.trim()) {
      setMessage({
        type: "error",
        text: "Por favor, preencha a URL do produto.",
      });
      setLoading(false);
      return;
    }
    ...
    if (!currentPrice.trim()) {
      setMessage({ type: "error", text: "Por favor, preencha o preço atual." });
      setLoading(false);
      return;
    }
      ...
      >
        ...
        >
          ...
          <small style={{ color: "var(--text-light)", fontSize: "0.8rem" }}>
            Dica: Cole o link e clique fora do campo para preencher
            automaticamente.
          </small>
        </div>
